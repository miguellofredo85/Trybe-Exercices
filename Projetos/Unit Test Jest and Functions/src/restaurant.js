...
// COM AJUDA E EXPLICACAO DE GRANDE MESTRE GUSTAVO SANTOS

let base = {};
const order = (str) => base.consumption.push(str);

const pay = () => {
  let conta = 0;
  let drinkis = Object.values(base.fetchMenu().drink);
  let foodis = Object.values(base.fetchMenu().food);
  for (let index = 0; index < drinkis.length; index += 1) {
    conta += drinkis[index];
  }
  for (let index = 0; index < foodis.length; index += 1) {
    conta += foodis[index]; 
  }
  // let contaFinal = conta *1.1;
  return conta * 1.1;
};
